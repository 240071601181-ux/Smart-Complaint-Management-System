...
export function AppLayout({ children, path }: { children: React.ReactNode; path: string }) {
  ...
  return <div className={`app-shell ${collapsed ? "sidebar-collapsed" : ""}`}>
    <aside className={`sidebar ${mobileOpen ? "mobile-open" : ""}`}>
      ...
      <nav className="sidebar-nav">{navGroups.map(group => <div className="nav-group" key={group.label}><span className="nav-group-label">{group.label}</span>{group.items.map(item => { const active = path === item.path || (path.startsWith("/leads/") && item.path === "/leads") || (path.startsWith("/calls/") && item.path === "/calls"); return <button key={item.path} onClick={() => { navigate(item.path); setMobileOpen(false); }} className={`nav-item ${active ? "active" : ""}`} title={collapsed ? item.label : undefined}><item.icon size={17} /><span>{item.label}</span>{item.badge && <em>{item.badge}</em>}</button> })}</div>)}</nav>
