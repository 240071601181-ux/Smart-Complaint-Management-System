...
import { AmbientShards, Button, Card, IconView, MetricCard, TierBadge } from "@/components/app/ui";
 ...
 * - Recent leads table: DEMO rows, but each row links to /leads/:id, which
 *   resolves live-or-mock through the 14C-4 lead integration. No per-row
 *   backend fetch happens here (mock ids would only 404).
 ...
 *   only — it does not verify individual services). This is the sole
 *   real-API connection on this page; no /api/v1/dashboard or /analytics
...
function Dashboard({ onToast }: { onToast: (message: string) => void }) {
  ...
  return <><AmbientShards variant="dashboard" />
    <div className="page-heading"><div><p className="lede">{pageMeta["/dashboard"].description}</p></div><div className="heading-actions"><Button icon={Plus} variant="secondary" onClick={() => navigate("/leads/new")}>Create lead</Button><Button icon={Mic2} variant="primary" onClick={() => onToast("AI voice agent is connecting…")}>Start AI call</Button></div></div>
    <div className="metric-grid"><MetricCard label="Total leads" value="1,284" delta="12.8%" note="vs. previous 30 days" icon={Users} /><MetricCard label="Hot leads" value="186" delta="8.4%" note="14.5% of pipeline" accent="violet" icon={Target} /><MetricCard label="Calls today" value="248" delta="18.2%" note="93% connection rate" accent="amber" icon={Phone} /><MetricCard label="Qualification rate" value="72.4%" delta="6.1%" note="+4.2 pts this week" accent="green" icon={Gauge} /></div>
    ...
    <div className="dashboard-grid bottom-grid"><Card className="leads-card"><div className="card-header"><div><span className="section-kicker">LIVE PIPELINE</span><h2>Recent leads</h2></div><button className="link-btn" onClick={() => navigate("/leads")}>View all <ArrowUpRight size={14} /></button></div><div className="table-wrap"><table><thead><tr><th>Lead</th><th>Route</th><th>Signal</th><th>Status</th><th>Last contact</th></tr></thead><tbody>{leads.slice(0, 4).map(lead => <tr key={lead.id} onClick={() => navigate(`/leads/${lead.id}`)}><td><div className="person-cell"><span className="person-avatar" style={{ background: `${lead.color}18`, color: lead.color }}>{lead.initials}</span><span><b>{lead.name}</b><small>{lead.company}</small></span></div></td><td>{lead.route}</td><td><div className="score-cell"><TierBadge tier={lead.tier} /><b>{lead.score}</b></div></td><td><span className="status-pill"><i />{lead.status}</span></td><td className="muted">{lead.last}</td></tr>)}</tbody></table></div></Card><Card className="activity-card"><div className="card-header"><div><span className="section-kicker">SYSTEM PULSE</span><h2>Live activity</h2></div><span className="live-chip"><i />Live</span></div><div className="activity-list">{[["AI call completed", "Meera Shah · 92 sec", "2m", "cyan", Phone],["Lead qualified HOT", "Arjun Rao · score 92", "8m", "violet", Target],["WhatsApp delivered", "PS Pharma · template 04", "12m", "green", MessageCircle],["Follow-up scheduled", "VK Industrial · tomorrow", "18m", "amber", Clock3]].map(([title, sub, time, color, I]) => <div className="activity-item" key={title as string}><span className={`activity-icon ${color}`}><IconView icon={I as IconType} size={14} /></span><span className="activity-copy"><b>{title as string}</b><small>{sub as string}</small></span><time>{time as string}</time></div>)}</div><button className="activity-footer" onClick={() => navigate("/activity")}>Open activity center <ArrowUpRight size={14} /></button></Card></div>
    <Card className="health-card"><div className="health-copy"><span className="section-kicker">RUNTIME HEALTH</span><h2>Everything is flowing</h2><p>Voice, CRM, messaging, and calendar services are connected and responding normally.</p><p className="lede" style={{ marginTop: 6 }}>{healthLine}</p></div><div className="health-items"><div><span className="health-icon green"><Phone size={15} /></span><span><b>Voice gateway</b><small>Operational · 182ms</small></span><i className="health-check"><Check size={12} /></i></div><div><span className="health-icon cyan"><Database size={15} /></span><span><b>CRM sync</b><small>Synced 2 min ago</small></span><i className="health-check"><Check size={12} /></i></div><div><span className="health-icon violet"><MessageCircle size={15} /></span><span><b>WhatsApp Cloud</b><small>Operational · 99.8%</small></span><i className="health-check"><Check size={12} /></i></div></div></Card>
  </>;
}
