...
function LeadsPage({ onToast }: { onToast: (message: string) => void }) {
  ...
  const rows = useMemo(
    () =>
      (list.data?.leads ?? [])
        .map(toDisplayLead)
        .filter((l) => tier === "All signals" || l.tier === tier),
    [list.data, tier]
  );
